...
bool loadFromLittleFS(String path) {
  ...
  if (path.endsWith("index.html") && !LittleFS.exists(path)) {
...
void setup(void)
{
  ...
  //Load config values from LittleFS to replace default ones
  ...
  if (LittleFS.exists("/pk.pem") && LittleFS.exists("/cert.der"))
  {
    ...
    certFile.close();
    File pkFile = LittleFS.open("/pk.pem", "r");
    klen = pkFile.size();
    ...
    if (createCertResult != 0) {
      ...
      File pkFile = LittleFS.open("/pk.pem", "w");
      pkFile.write( cert->getPKData(), cert->getPKLength());
      pkFile.close();
      File certFile = LittleFS.open("/cert.der", "w");
